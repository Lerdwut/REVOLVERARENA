...
The UI-01 through UI-03 runtime polish pass is implemented in the `feat/ui-01-03-polish` branch. Code-level validation is complete; Roblox Studio Play Mode remains the final visual and interaction check for HUD readability, lobby state transitions, compact viewport scaling, settings input blocking, and respawn cleanup.
