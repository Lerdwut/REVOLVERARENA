...
There is no `StarterGui` mapping. Runtime UI scripts create their `ScreenGui` instances under each player's `PlayerGui`. Reusable Fusion components and UI Labs stories live under `ReplicatedStorage.UI`; stories are development previews and must not be required by gameplay entry points. `Packages` and `DevPackages` are generated by Wally and should not be edited or committed.
...
For reusable Fusion components and UI Labs previews, follow the [Fusion/UI Labs workflow](ui-workflow.md).
